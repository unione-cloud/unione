...
public class DataBaseDao {
	...
	private <T> SqlId loadSql(SqlBuilder<T> builder,SqlType type) {
		...
		if(this.sqlManager.containSqlId(sqlid)) {
			...
			if(tempSource==null) {
				tempSource=new SQLSource(sqlid, this.buildSql(builder, type));
	...
	private <T> String buildSql(SqlBuilder<T> builder,SqlType type) {
		...
		if("SELECT".equalsIgnoreCase(type.value())) {
			...
			sqlEntity.getFieldList().stream().forEach(field->{
				fieldBuf.append(",").append(field);
			});
			...
			buffer.append(sqlEntity.getTable()).append(" SET \n")
			.append("-- @sqlTrim(){\n");
			
			sqlEntity.getFields().stream().forEach(field->{
				if(field.getBaseField()!=null && (BaseField.LAST_UPDATED.name().equals(field.getBaseField().name()) || 
						BaseField.LAST_UPDATED_BY.name().equals(field.getBaseField().name()))) {
					return;
				}
			...
			sqlEntity.getBaseFields().stream().forEach(field->{
				if(BaseField.LAST_UPDATED.name().equals(field.getBaseField().name()) || 
						BaseField.LAST_UPDATED_BY.name().equals(field.getBaseField().name())) {
