...
public class SysPageReleaseController implements PojoFeignApi<SysPageRelease>{
	...
	@Override
	public Results<Long> save(@Validated(Validator.save.class) SysPageRelease entity) {
		log.debug("进入:新增页面发布信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增页面发布");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增页面发布信息.entity:{},result:true",entity);
	...
	public Results<Long> update(@Validated(Validator.update.class) SysPageRelease entity) {
		...
		String[] fields = {"appId","tmplId","title","sn","vers","summary","icon","picMax","picMid","picMix","types","trades","reviewPic","configs","isTmpl","isGlobal","status","descs"};
		...
		log.debug("退出:修改页面发布信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除页面发布信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除页面发布");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysPageRelease.class,ids));
		...
		log.debug("退出:删除页面发布信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
