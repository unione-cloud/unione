...
public class LoginService {
	 ...
	 * @return
	 */
	public LoginResult doLoginByOAuth(String type, String code) {
	...
	private LoginResult loginSuccess(SysUser user) {
		...
		SqlBuilder<SysRole> loadUserRole = SqlBuilder
				.build(role)
				.query("SELECT r.SN FROM SYS_USER_ROLE ur LEFT JOIN SYS_ROLE r ON ur.ROLE_ID=r.ID WHERE r.STATUS=1 AND ur.USER_ID=#{params.userId}");
		List<SysRole> roles = dataBaseDao.findList(loadUserRole);
		Set<String> roleCodes = roles.stream().map(row -> row.getSn()).filter(row -> row != null)
