...
public class SysGroupController implements TreeFeignApi<SysGroup>{
	...
	public Results<Long> save(@Validated(Validator.save.class) SysGroup entity) {
		log.debug("进入:新增分组信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增分组");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增分组信息.entity:{},result:true",entity);
	...
	public Results<Long> update(@Validated(Validator.update.class) SysGroup entity) {
		...
		log.debug("退出:修改分组信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<Long> delete(Set<Long> ids){
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysGroup.class,ids));
