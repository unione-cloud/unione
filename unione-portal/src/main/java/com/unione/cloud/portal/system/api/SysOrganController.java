...
@Slf4j
@RestController
@Api(tags = "SysOrgan sys_organ管理：系统管理：机构信息",description="SysOrgan")
@RequestMapping("/api/system/sysOrgan")	 //TreeFeignApi
public class SysOrganController implements PojoFeignApi<SysOrgan>{
	...
	public Results<List<SysOrgan>> find(Params<SysOrgan> params) {
		log.debug("进入:查询sys_organ列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询sys_organ列表");
		...
		LogsUtil.success();
		log.debug("退出:查询sys_organ列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) SysOrgan entity) {
		log.debug("进入:新增sys_organ信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增sys_organ");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增sys_organ信息.entity:{},result:true",entity);
		return Results.success(entity.getId());
	}


	@Override
	public Results<Long> update(@Validated(Validator.update.class) SysOrgan entity) {
		log.debug("进入:修改sys_organ信息方法，entity:{}",entity);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Modify, "修改sys_organ",entity.getId());
		...
		log.debug("退出:修改sys_organ信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysOrgan>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询sys_organ信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询sys_organ");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询sys_organ信息方法，ids:{},result:true",ids);
	...
	public Results<SysOrgan> detail(Long id) {
		log.debug("进入:查看sys_organ详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看sys_organ详细",id);
		// 参数处理
		...
		log.debug("退出:查看sys_organ详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除sys_organ信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除sys_organ");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysOrgan.class,ids));
		...
		log.debug("退出:删除sys_organ信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
...
//		log.debug("进入:加载下级sys_organ信息,sid:{}",sid);
//		LogsUtil.set(LogType.Query, "加载下级sys_organ信息",sid);
//		 //参数处理
...
//		LogsUtil.add("下级sys_organ记录数量:"+rows.size());
//		
//		LogsUtil.success();
//		log.debug("退出:加载下级sys_organ信息,sid:{},result:true",sid);
//		return Results.success(rows);
//	}
