...
public class BaseDictSettingController implements PojoFeignApi<BaseDictSetting>{
	...
	public Results<List<BaseDictSetting>> find(Params<BaseDictSetting> params) {
		log.debug("进入:查询base_dict_setting列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询base_dict_setting列表");
		...
		LogsUtil.success();
		log.debug("退出:查询base_dict_setting列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) BaseDictSetting entity) {
		log.debug("进入:新增base_dict_setting信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增base_dict_setting");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增base_dict_setting信息.entity:{},result:true",entity);
		return Results.success(entity.getId());
	}


	@Override
	public Results<Long> update(@Validated(Validator.update.class) BaseDictSetting entity) {
		log.debug("进入:修改base_dict_setting信息方法，entity:{}",entity);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Update, "修改base_dict_setting",entity.getId());
		...
		log.debug("退出:修改base_dict_setting信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<BaseDictSetting>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询base_dict_setting信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询base_dict_setting");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询base_dict_setting信息方法，ids:{},result:true",ids);
	...
	public Results<BaseDictSetting> detail(Long id) {
		log.debug("进入:查看base_dict_setting详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看base_dict_setting详细",id);
		// 参数处理
		...
		log.debug("退出:查看base_dict_setting详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除base_dict_setting信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除base_dict_setting");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(BaseDictSetting.class,ids));
		...
		log.debug("退出:删除base_dict_setting信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}


//	@Override
//	public Results<List<BaseDictSetting>> children(Long sid){
//		log.debug("进入:加载下级base_dict_setting信息,sid:{}",sid);
//		LogsUtil.set(LogType.Query, "加载下级base_dict_setting信息",sid);
//		 //参数处理
//		AssertUtil.service().notNull(sid, "参数sid不能为空");
//		
//		// 执行查询
//		BaseDictSetting params = new BaseDictSetting();
//		params.setParentId(sid);
//		LogsUtil.add("parentId:%s",sid);
//	
//		List<BaseDictSetting> rows = dataBaseDao.findList(SqlBuilder.build(params));
//		LogsUtil.add("下级base_dict_setting记录数量:"+rows.size());
//		
//		LogsUtil.success();
//		log.debug("退出:加载下级base_dict_setting信息,sid:{},result:true",sid);
//		return Results.success(rows);
//	}
