...
public class BaseDictSettingController implements PojoFeignApi<BaseDictSetting>{
	...
	public Results<List<BaseDictSetting>> find(Params<BaseDictSetting> params) {
		log.debug("进入:查询字典设置列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询字典设置列表");
		...
		LogsUtil.success();
		log.debug("退出:查询字典设置列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) BaseDictSetting entity) {
		log.debug("进入:保存字典设置.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "保存字典设置");
		// 参数处理
		int len = 0;
	...
	public Results<List<BaseDictSetting>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询字典设置信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询字典设置");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询字典设置信息方法，ids:{},result:true",ids);
	...
	public Results<BaseDictSetting> detail(Long id) {
		log.debug("进入:查看字典设置详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看字典设置详细",id);
		// 参数处理
		...
		log.debug("退出:查看字典设置详细信息方法，id:{},result:true",id);
	...
	public Results<Integer> delete(Set<Long> ids){
		log.debug("进入:删除字典设置信息方法，ids:{}",ids);
		Results<Integer> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除字典设置");
		...
		log.debug("退出:删除字典设置信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
