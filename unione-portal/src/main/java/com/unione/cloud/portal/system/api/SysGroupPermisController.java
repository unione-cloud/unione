...
@Slf4j
@RestController
@Api(tags = "系统管理：分组权限",description="SysGroupPermis")
...
public class SysGroupPermisController implements PojoFeignApi<SysGroupPermis>{
	...
	public Results<List<SysGroupPermis>> find(Params<SysGroupPermis> params) {
		log.debug("进入:查询分组权限列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询分组权限列表");
		...
		LogsUtil.success();
		log.debug("退出:查询分组权限列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) SysGroupPermis entity) {
		log.debug("进入:新增分组权限信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增分组权限");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增分组权限信息.entity:{},result:true",entity);
	...
	public Results<Long> update(@Validated(Validator.update.class) SysGroupPermis entity) {
		...
		log.debug("退出:修改分组权限信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysGroupPermis>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询分组权限信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询分组权限");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询分组权限信息方法，ids:{},result:true",ids);
	...
	public Results<SysGroupPermis> detail(Long id) {
		log.debug("进入:查看分组权限详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看分组权限详细",id);
		// 参数处理
		...
		log.debug("退出:查看分组权限详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除分组权限信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除分组权限");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysGroupPermis.class,ids));
		...
		log.debug("退出:删除分组权限信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
