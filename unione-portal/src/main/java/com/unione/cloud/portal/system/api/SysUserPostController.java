...
public class SysUserPostController implements PojoFeignApi<SysUserPost>{
	...
	public Results<List<SysUserPost>> find(Params<SysUserPost> params) {
		log.debug("进入:查询用户岗位列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询用户岗位列表");
		...
		LogsUtil.success();
		log.debug("退出:查询用户岗位列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) SysUserPost entity) {
		log.debug("进入:新增用户岗位信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增用户岗位");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增用户岗位信息.entity:{},result:true",entity);
		return Results.success(entity.getId());
	}


	@Override
	public Results<Long> update(@Validated(Validator.update.class) SysUserPost entity) {
		log.debug("进入:修改用户岗位信息方法，entity:{}",entity);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Update, "修改用户岗位",entity.getId());
		...
		log.debug("退出:修改用户岗位信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysUserPost>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询用户岗位信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询用户岗位");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询用户岗位信息方法，ids:{},result:true",ids);
	...
	public Results<SysUserPost> detail(Long id) {
		log.debug("进入:查看用户岗位详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看用户岗位详细",id);
		// 参数处理
		...
		log.debug("退出:查看用户岗位详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除用户岗位信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除用户岗位");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysUserPost.class,ids));
		...
		log.debug("退出:删除用户岗位信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
