...
@Slf4j
@RestController
@Api(tags = "系统管理：岗位权限",description="SysPostPermis")
...
public class SysPostPermisController implements PojoFeignApi<SysPostPermis>{
	...
	public Results<List<SysPostPermis>> find(Params<SysPostPermis> params) {
		log.debug("进入:查询岗位权限列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询岗位权限列表");
		...
		LogsUtil.success();
		log.debug("退出:查询岗位权限列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) SysPostPermis entity) {
		log.debug("进入:新增岗位权限信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增岗位权限");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增岗位权限信息.entity:{},result:true",entity);
	...
	public Results<Long> update(@Validated(Validator.update.class) SysPostPermis entity) {
		...
		log.debug("退出:修改岗位权限信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysPostPermis>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询岗位权限信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询岗位权限");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询岗位权限信息方法，ids:{},result:true",ids);
	...
	public Results<SysPostPermis> detail(Long id) {
		log.debug("进入:查看岗位权限详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看岗位权限详细",id);
		// 参数处理
		...
		log.debug("退出:查看岗位权限详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除岗位权限信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除岗位权限");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysPostPermis.class,ids));
		...
		log.debug("退出:删除岗位权限信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
