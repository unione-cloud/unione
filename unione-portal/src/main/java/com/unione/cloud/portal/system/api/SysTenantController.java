...
@Slf4j
@RestController
@Api(tags = "系统管理：租户信息",description="SysTenant")
...
public class SysTenantController implements PojoFeignApi<SysTenant>{
	...
	public Results<List<SysTenant>> find(Params<SysTenant> params) {
		log.debug("进入:查询租户信息列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询租户信息列表");
		...
		LogsUtil.success();
		log.debug("退出:查询租户信息列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysTenant>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询租户信息信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询租户信息");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询租户信息信息方法，ids:{},result:true",ids);
	...
	public Results<SysTenant> detail(Long id) {
		log.debug("进入:查看租户信息详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看租户信息详细",id);
		// 参数处理
		...
		log.debug("退出:查看租户信息详细信息方法，id:{},result:true",id);
	...
	public Results<Long> delete(Set<Long> ids){
		log.debug("进入:删除租户信息信息方法，ids:{}",ids);
		Results<Long> results = new Results<>();
		LogsUtil.set(LogType.Delete, "删除租户信息");
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysTenant.class,ids));
		...
		log.debug("退出:删除租户信息信息方法，ids:{},result:{}",ids,results.isSuccess());
		return results;
	}
