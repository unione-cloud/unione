...
public class SysPostController implements TreeFeignApi<SysPost>{
	...
	public Results<Long> update(@Validated(Validator.update.class) SysPost entity) {
		...
		String[] fields = {"orgId","parentId","name","sn","lvsn","level","types","iconFont","iconPic","duty","descs","isLeaf","ordered","status","delFlag"};
	...
	public Results<Long> delete(Set<Long> ids){
		...
		int count = dataBaseDao.delete(SqlBuilder.build(SysPost.class,ids));
