...
public class SysUserRoleController implements PojoFeignApi<SysUserRole>{
	...
	public Results<List<SysUserRole>> find(Params<SysUserRole> params) {
		log.debug("进入:查询用户角色列表方法,params:{}",params);
		LogsUtil.set(LogType.Query, "查询用户角色列表");
		...
		LogsUtil.success();
		log.debug("退出:查询用户角色列表方法,params:{},result:{}",params,results.isSuccess());
		return results;
	}
	...
	public Results<Long> save(@Validated(Validator.save.class) SysUserRole entity) {
		log.debug("进入:新增用户角色信息.entity:{}",entity);
		LogsUtil.set(LogType.Insert, "新增用户角色");
		// 参数处理
		dataBaseDao.insert(entity);
		
		LogsUtil.success(entity.getId());
		log.debug("退出:新增用户角色信息.entity:{},result:true",entity);
	...
	public Results<Long> update(@Validated(Validator.update.class) SysUserRole entity) {
		...
		log.debug("退出:修改用户角色信息方法，entity:{},result:{}",entity,results.isSuccess());
		return results;
	}
	...
	public Results<List<SysUserRole>> findByIds(Set<Long> ids) {
		log.debug("进入:批量查询用户角色信息方法，ids:{}",ids);
		LogsUtil.set(LogType.Query, "批量查询用户角色");
		// 参数处理
		...
		LogsUtil.success();
		log.debug("退出:批量查询用户角色信息方法，ids:{},result:true",ids);
	...
	public Results<SysUserRole> detail(Long id) {
		log.debug("进入:查看用户角色详细信息方法，id:{}",id);
		LogsUtil.set(LogType.Query, "查看用户角色详细",id);
		// 参数处理
		...
		log.debug("退出:查看用户角色详细信息方法，id:{},result:true",id);
