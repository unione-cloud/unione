...
@Slf4j
@RestController
@Tag(name = "统一消息：消息分类",description="UmsCategory")
@RequestMapping("/api/ums/category")	 //TreeFeignApi
public class UmsCategoryController implements PojoFeignApi<UmsCategory>{
	...
	@Override
	@Action(title="保存消息分类",type = ActionType.Save)
	...
	@PostMapping("/status")
	@Action(title="设置分类状态",type = ActionType.Save,roles = {UserRoles.SYSOPSUSER})
	@Operation(summary = "设置分类状态", description="USEORNOT 1 使用，0停用")
	public Results<Void> setStatus(@RequestBody UmsCategory entity){
	...
	@Override
	@Action(title="加载消息分类列表",type = ActionType.Query,nolog = true)
	public Results<List<UmsCategory>> findByIds(Set<Long> ids) {
		// 参数处理
		AssertUtil.service().isTrue(!ids.isEmpty(), "参数ids不能为空");
		
		List<UmsCategory> rows = dataBaseDao.findByIds(SqlBuilder.build(UmsCategory.class,new ArrayList<>(ids)));
	...
	@Override
	@Action(title="加载消息分类详情",type = ActionType.Query,nolog = true)
	public Results<UmsCategory> detail(Long id) {
		// 参数处理
		AssertUtil.service().notNull(id,"参数id不能为空");
		
		UmsCategory tmp = dataBaseDao.findById(SqlBuilder.build(UmsCategory.class,id));
	...
	@Override
	@Action(title="删除消息分类",type = ActionType.Delete)
