...
import com.unione.cloud.system.dto.OrganPermisDto;
...
/**
 * @标题 	OrganPermisDto Controller 服务
...
@Slf4j
@RestController
@Tag(name = "系统管理：机构权限",description="OrganPermisDto")
@RequestMapping("/api/system/organPermis")	 //TreeFeignApi
public class SysOrganPermisController implements PojoFeignApi<OrganPermisDto>{
	...
	@Override
	@Action(title="查询机构权限",type = ActionType.Query)
	public Results<List<OrganPermisDto>> find(Params<OrganPermisDto> params) {
		AssertUtil.service().notNull(params.getBody(),"请求参数body不能为空");
				
		Results<List<OrganPermisDto>> results = dataBaseDao.findPages(params);
			...
			if(entity.getOrgId()!=null && entity.getResId()!=null){
				...
				if(entity.getOrgId()!=null){
					...
					if(entity.getAddPermis()!=null){
						...
						// 过滤已存在的权限
						List<SysOrganPermis> adds = entity.getAddPermis().stream().filter(row->!hdMap.contains(row.getResId())).map(row->{
							BeanUtils.setDefaultValue(row, "enDilivery",0);
							row.setOrgId(entity.getOrgId());
							return row;
						}).collect(Collectors.toList());
						LogsUtil.add("新增机构权限，数量：%s,可新增:%s",entity.getAddPermis().size(),adds.size());
						if(adds.size()>0) {
					...
					if(entity.getAddPermis()!=null){
						// 加载当前资源已有机构列表
						Set<Long> uids = entity.getAddPermis().stream().map(row->row.getOrgId()).collect(Collectors.toSet());
						Set<Long> hdMap=new HashSet<>();
						if(uids.size()>0) {
							dataBaseDao.findList(SqlBuilder.build(SysOrganPermis.class)
						...
						// 过滤已存在的权限
						List<SysOrganPermis> adds = entity.getAddPermis().stream().filter(row->!hdMap.contains(row.getOrgId())).map(row->{
							BeanUtils.setDefaultValue(row, "enDilivery",0);
	...
	@Override
	public Results<List<OrganPermisDto>> findByIds(Set<Long> ids) {
		// 参数处理
		AssertUtil.service().isTrue(!ids.isEmpty(), "参数ids不能为空");
		
		List<OrganPermisDto> rows = dataBaseDao.findByIds(SqlBuilder.build(OrganPermisDto.class,new ArrayList<>(ids)));
		return Results.success(rows);
	}


	@Override
	public Results<OrganPermisDto> detail(Long id) {
		// 参数处理
		AssertUtil.service().notNull(id,"参数id不能为空");
		
		OrganPermisDto tmp = dataBaseDao.findById(SqlBuilder.build(OrganPermisDto.class,id));
	...
	@Override
	@Action(title="删除机构权限",type = ActionType.Delete,roles = {UserRoles.ORGANADMIN,UserRoles.SYS3PAUTH})
	public Results<Integer> delete(Set<Long> ids){
		...
		AssertUtil.service().isTrue(!ids.isEmpty(), "参数ids不能为空");
		// 执行删除
		int count = dataBaseDao.deleteById(SqlBuilder.build(OrganPermisDto.class,ids));
